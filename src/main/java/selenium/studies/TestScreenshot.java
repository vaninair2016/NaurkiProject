...
public class TestScreenshot {
	...
	public static void getScreenShot(WebDriver driver,String m) throws IOException {
    Date d = new Date();
    String scFormat=d.toGMTString().toString().replace(" ", "_").replace(":", "_");
    
	File sc=	((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
	String userDir= System.getProperty("user.dir");
	String targetPath= userDir+"\\screenshots\\"+scFormat+"_"+m+".png";
	FileUtils.copyFile(sc, new File(targetPath));
	}
	public static void main(String[] args) throws InterruptedException, IOException {
		TestScreenshot tc = new TestScreenshot();
		setUp();
		//tc.getScreenShot();
		tearDown();
	}
